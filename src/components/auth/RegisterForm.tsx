      ...
      if (error instanceof AxiosError) {
        if (error.response && error.response.data) {
          ...
          if (serverError.errors) {
            ...
            setErrors((prev) => ({ ...prev, general: serverError.message || "Lỗi từ server." }));
          }
        } else {
  ...
  return (
    <AuthForm title="Đăng Ký" onSubmit={handleSubmit}>
      <div className="w-full bg-white shadow-xl rounded-2xl px-6 py-8">
        ...
        <button type="submit" className="w-full bg-blue-500 text-white py-3 rounded-xl mt-4" disabled={isPending}>
